...
from token_detection import detection_task, back_detection_task
...
if __name__ == '__main__':
    ...
    print("  ReadFrontCamera: 5ms   | HIGH   — feeds brightness + front detection")
    ...
    print("  DetectionTask  : 10ms  | MEDIUM — front cv2: coins + police car")
    print("  BackDetectTask : 10ms  | MEDIUM — Ch2: chasing car (isolated from front cv2)")
    print("  EventsWatchdog : 100ms | LOW    — Ch2+Ch3 deadline countdown\n")
    ...
    t_back_detection  = RTTask("BackDetectTask",  period=0.010, priority=TaskPriority.MEDIUM, execute_func=back_detection_task)
    t_police_watchdog = RTTask("EventsWatchdog",  period=0.100, priority=TaskPriority.LOW,    execute_func=police_watchdog_task)
