...
shared_data = {
    ...
    # Token detection
    'last_token': None,        # 'green', 'red', 'yellow', None
    'event_active': None,      # 'trailing_car', 'police', 'low_brightness', None
    'corrupted_frame': False,  # True when yellow corruption event is active
    'steering_cooldown': 0.0,  # timestamp until steering tap finishes
    # Dodge / lane state
    'dodge_direction': 0.0,    # current active dodge steer (-1.0, 0.0, or 1.0)
    'dodge_until': 0.0,        # timestamp until active dodge holds
    'lane_offset': 0.0,        # accumulated lane offset (-1=left, 0=centre, +1=right)
}
...
def processing_task():
    ...
    # --- Low Brightness Detection (event: turn light ON) ---
    ...
    if brightness < 60:
        ...
        with data_lock:
            if shared_data.get('event_active') == 'low_brightness':
...
def send_controls_task():
    ...
    # Always force full acceleration
